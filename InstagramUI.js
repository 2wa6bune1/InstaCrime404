...
class InstagramUI {
  ...
  update(appMouse) {
    ...
    let isMonoActive = (typeof monologue !== 'undefined' && monologue.active);
    ...
    if (this.currentScreen === "story") {
      if (this.isTransitioning) {
        ...
        let currentS = this.stories[this.currentStory];
        if (currentS && currentS.monoText && !currentS.monoPlayed) {
          monologue.start(currentS.monoText);
          currentS.monoPlayed = true;
        }
  ...
  display(appMouse = { x: -999, y: -999 }) {
    ...
    } else if (this.currentScreen === "dmList" || this.currentScreen === "chatRoom") {
      // 💡 분리된 DM 모듈에 렌더링 위임
      this.dm.display(appMouse);
    }
  }
  ...
  displayBottomNav(appMouse) {
    ...
    for (let i = 0; i < xs.length; i++) {
      ...
      if (i === 0) {
        ...
        let unread = this.dm.getUnreadCount();
        if (unread > 0) {
          push();
          noStroke();
          fill(255, 60, 90);
  ...
  handleClick(mx, my) {
    ...
    // 💡 핵심: DM 리스트나 채팅방일 때 DM 모듈의 handleClick을 호출
    else if (this.currentScreen === "dmList" || this.currentScreen === "chatRoom") {
      this.dm.handleClick(mx, my);
    }
  }

  handleWheel(event) {
    if (this.currentScreen === "dmList" || this.currentScreen === "chatRoom") {
      this.dm.handleWheel(event);
      return;
    }
